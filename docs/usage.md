...
MCP server 明确返回的 JSON-RPC 工具错误会作为普通工具结果回注模型。共享传输关闭时，
Formic 同样把“原调用结局未知且未重放”作为不可缓存的工具错误回注，使模型改用其他工具；
它不得重复可能产生副作用的原调用。调用超时、会话建立失败和本地完成后处理失败仍会终止
当前 worker。
...
| --- | --- |
| `0` | 本次作业完整，没有未完成单元 |
| `1` | 存在 failed、stopped 或 not_started |
| `2` | 启动配置、输入或续跑一致性无效 |
| `3` | 收到终止信号 |
