...
- 带自然序号及相对毫秒时间的状态时间线；
- 可逐字重建的 LLM 请求、按调用合并的原始 SSE 负载或明确的流超限记录、工具参数与结果；
...
一次未触发流量上限的 LLM 调用，其全部 SSE data 负载保存在同一个有序 JSON 数组中。
数组元素仍是供应商返回的原始字符串，事件边界和顺序不变；Markdown 只为整次响应生成一个
时间线标题，不再为每个 token 增量生成标题。响应流超过 64 MiB 时，档案保存此前全部字节，
并为触发超限的网络块保存最多 64 KiB 原始前缀以及原始长度、遗漏长度、超限量和编码。
