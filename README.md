...
Formic 让模型通过内部提交工具交付 object，通过本地 schema 校验后发布 JSON。无效提交未达到配置上限时，模型收到全部校验问题并继续修正；达到上限时本单元失败且不发布 JSON。支持的 schema 子集、计数方式和可复制示例见[结构化输出](docs/usage.md#9-结构化输出)。
...
完成启动准备后，每次执行创建新的 `run-N`。结果先写临时文件，再原子发布；已存在的完成记录保留。MCP 等启动故障的恢复步骤见[排错说明](docs/observability.md#71-作业启动失败)。
